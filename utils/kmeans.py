...
class KMEANS:
    def __init__(self, n_clusters=5, max_iter=None, verbose=True,device = torch.device("cpu")):
    ...
    def fit(self, x):
        ...
        while True:
            ...
            # 更新中心点 对上一步中已经聚类标记好的一簇重新取平均计算聚类中心
    ...
    def nearest_center(self, x):
        ...
        self.labels = labels
        if self.started:
            self.variation = torch.sum(self.dists - dists)   # 如果两次变化的距离小于一定的范围 则认为已经够了
        self.dists = dists
        self.started = True
...
def time_clock(matrix,device):
    a = time.time()
    k = KMEANS(max_iter=10,verbose=True,device=device)
    k.fit(matrix)
    b = time.time()
    return (b-a)/k.count
...
if __name__ == "__main__":
    ...
    cpu_speeds = []
    for i in tqdm([2,20,100,500,2000,8000,20000]):  # 数据的维数
