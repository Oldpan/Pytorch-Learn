...
This tool is based on Pytorch Framework.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='find the pretrained pytorch models int8 quantize scale value')
    ...
    parser.add_argument('--exam-images', dest='exam-images',
                        help='path to calibration exam-images', type=str)
...
def add_to_distribution(blob, distribution, interval):
    ...
    Args:
        blob: the output blob of pytorch layer
...
def net_forward(net, image_path, transformer):
    ...
    """
    # load image
    image = pytorch.io.load_image(image_path)
...
def network_prepare(net, mean, norm):
    ...
    # initial transformer
    transformer = pytorch.io.Transformer({'data': net.blobs['data'].data.shape})
...
def weight_quantize(net, net_file):
    ...
    Args:
        net: the instance of pytorch inference
        net_file: deploy pytorch prototxt
    Returns:    
        none
    """
...
def activation_quantize(net, transformer, images_files):
    ...
    for i, blob_name in enumerate(top_blob_names):
        ...
        print("%-20s max_val : %-10.8f distribution_intervals : %-10.8f" % (
            blob_name, max_vals[i], distribution_intervals[i]))
    ...
    for i, distribution in enumerate(distributions):
        ...
        print("%-20s bin : %-8d threshold : %-10f interval : %-10f scale : %-10f" % (
            top_blob_names[i], threshold_bin, threshold, distribution_intervals[i], calibration_val))
...
def usage_info():
    ...
    print("try it again:\n python pytorch-int8-convert.py -h")
...
def main():
    ...
    if args.model is None or args.mean is None or args.images is None:
        usage_info()
        return None
    ...
    if args.gpu != 0:
        pytorch.set_device(0)
        pytorch.set_mode_gpu()

    # initial pytorch net and the forword model(GPU or CPU)
    net = pytorch.Net(net_file, caffe_model, pytorch.TEST)

    # prepare the cnn network
    transformer = network_prepare(net, mean, norm)

    # get the calibration datasets exam-images files path
    ...
    print(
        "\npytorch Int8 Calibration table create success,best wish for your INT8 inference has a low accuracy loss...\(^▽^)/")
